## A code package for modeling electrified space heating demands and analyzing the benefits and impacts of their flexibility potential on power systems in Switzerland and its neighboring countries – France, Germany, Italy, and Austria.

**Title:** *Impacts and Benefits of Electrified Space Heating on Renewable-Based Power Systems*  
**Journal:** Nature Communications  
**Submission ID:** NCOMMS-26-018997-T
...
assess its flexibility potential for power system operation from 2021 to 2050. The code package is structured into three major Modules:

- **HP demand and flexibility identification**  
    Simulates hourly heat pump (HP) electricity consumption for individual builings; Calculates hourly HP demand and flexibility bounds from building construction data and temperature information;

- **Power Systems Analysis Simulation**  
    Performs hourly simulations on Swiss power grids having interconnection with power systems in German, Italy, French and Austria, by dispatching the conventional thermal generators, renewable generations (i.e., solar, wind and hydro) , flexible HP demand, and power exchange between countries.It exames how large-scale electrified space heating affects the generation portfolio, peak demand, renewable curtailment, electricity prices, system operation and long-term investment costs, and cross-border energy exchanges between Switzerland and its neighboring countries - France, Germany, Italy, and Austria, in 2021, 2030, 2040, and 2050
...
## Scenario definition
The inputs and definition of all scenarios is summarized a table at the end of this README file.

## Module 1: HP demand and flexibility identification
This block calculates the HP hourly demand and flexibility bounds based on the building construction dataset. 
...
## Module 2: Power system simulations
We use the Nexuse simulation test to investigate the impacts and benefits of HP demand and flexibility on power systems. In this folder, we have include the basic software. The details of the Nexus-e can be found in the readme file
...
## Module 3: Output data and results
This folder contains the Jupyter notebooks used to analyse the processed model outputs and generate the figures included in the manuscript, the Extended Data, and the Supplementary Information.

The notebooks are organised by figure number. Each notebook reads pre-generated result files (mainly `.csv`, `.mat`, `.xlsx`, and `.shp` files), performs figure-specific post-processing, and exports the corresponding figures in SVG format.

This package is intended to allow editors and reviewers to reproduce the visualisations reported in the manuscript from the provided processed datasets. which contains the following notebooks:
...
The notebooks were saved with a Python 3 Jupyter kernel. The notebook metadata indicates Python **3.13.5** in the saved environment.  
To reproduce the figures, the following Python packages are required:
...
2. Place the processed input data in the folder structure expected by the notebooks.
3. Open Jupyter Notebook or JupyterLab.
4. Navigate to `Analysis and Visualization/`.
5. Run the notebook corresponding to the figure of interest from top to bottom (`Kernel -> Restart & Run All` is recommended).
...
All figures in the manuscript and associated supplementary materials can be reproduced by running the corresponding Jupyter notebooks provided in this package, provided that the processed input datasets are placed in the expected relative paths and the required Python dependencies are installed.

### Optional citation / code availability statement
The data analysis and visualisation code used to generate the figures in this study is provided as Jupyter notebooks in the submitted code package. The notebooks reproduce the manuscript figures from processed simulation outputs supplied alongside the code.
